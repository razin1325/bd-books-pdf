...
// All 22 Blogger image URLs mapped in EXACT serial order Page 01 to Page 22
const pages = [
...
async function main() {
  console.log('Verifying all 22 pages and downloading to D:\\HAND NOTE...');
  ...
  for (const p of pages) {
    ...
    console.log(`[${num}/22] Downloading ${p.name}...`);
    const res = await fetch(p.url);
    if (!res.ok) {
      console.error(`  ✗ Failed Page ${num}: HTTP ${res.status}`);
      continue;
    }
    const arrayBuffer = await res.arrayBuffer();
    const buffer = Buffer.from(arrayBuffer);
    fs.writeFileSync(path.join(destDir, p.name), buffer);
    console.log(`  ✓ Saved: ${p.name} (${(buffer.length / 1024).toFixed(1)} KB)`);
  }

  // Generate HTML content for lib/hsc-hand-note-data.ts
  const htmlBlocks = pages.map(p => {
    const num = String(p.page).padStart(2, '0');
...
`;
  ...
  console.log('\nSuccessfully updated lib/hsc-hand-note-data.ts with all 22 pages in exact serial order!');
}
